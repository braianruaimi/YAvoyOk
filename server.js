...
// ============================================
// 📧 EMAIL TRANSPORTER (OPCIONAL - NO BLOQUEA EL SERVIDOR)
// ============================================
...
app.use(corsConfig);
console.log('✅ CORS configurado - Orígenes:', process.env.ALLOWED_ORIGINS || 'localhost:5502');

// 3. Parseo de JSON y URL-encoded
app.use(express.json({ limit: '10mb' })); // Reducido de 50mb por seguridad
app.use(express.urlencoded({ extended: true, limit: '10mb' }));

// 4. Sanitización de inputs (previene inyección)
...
// 1. Autenticación (sin rate limit global, tiene su propio authLimiter)
...
// ============================================
// INICIALIZACIÓN DE DIRECTORIOS Y DATOS
// ============================================
...
app.post('/api/enviar-codigo-verificacion', async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({
        success: true,
        message: emailTransporter ? 'Código enviado exitosamente a tu email' : 'Código generado (email no configurado)',
...
// ============================================
// RUTAS PARA ARCHIVOS HTML (DESPUÉS DE TODAS LAS RUTAS API)
// ============================================
...
// Array para almacenar calificaciones ya declarado arriba en línea 61
...
// ============================================
// IMPORTANTE: El middleware de archivos estáticos DEBE estar DESPUÉS de todas las rutas API
// para evitar que intercepte las peticiones a /api/*
console.log('🔧 Configurando middleware de archivos estáticos...');

// Servir archivos estáticos desde el directorio raíz (styles.css, index.html, etc.)
app.use(express.static(__dirname, {
  maxAge: '1d',
  etag: true,
  index: false // No servir index automático, dejar que las rutas lo manejen
}));

// SERVIR TAMBIÉN DIRECTORIOS ESPECÍFICOS (compatibilidad)
...
inicializarDirectorios().then(() => {
  server.listen(PORT, () => {
    ...
    console.log(`📁 Registros: ${BASE_DIR}`);
    ...
    console.log('      GET  /api/mercadopago/public-key   - Clave pública');
