...
export function initForms() {
  ...
  if (formRepartidorInicio) {
    formRepartidorInicio.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        await fetch('/api/registrar-aceptacion-terminos', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            tipo: 'repartidor',
  ...
  if (formComercioInicio) {
    formComercioInicio.addEventListener('submit', async (e) => {
      ...
      const checkboxTerminos = document.getElementById('comercioAceptaTerminos');
      const errorTerminos = document.getElementById('errorTerminosComercio');
      
      if (!checkboxTerminos.checked) {
        if (errorTerminos) {
          errorTerminos.style.display = 'block';
          errorTerminos.textContent = '⚠️ Debes aceptar los Términos y Condiciones para continuar';
        }
      ...
      const cuit = document.getElementById('comercioCUIT')?.value.trim() || '';
      ...
      try {
        ...
        await fetch('/api/registrar-aceptacion-terminos', {
          ...
          body: JSON.stringify({
            tipo: 'comercio',
            nombre: `${nombrePropietario} - ${nombreComercio}`,
            email: email,
            telefono: telefono,
            documento: cuit,
            aceptaTerminos: true,
  ...
  if (formCrearPedido) {
    formCrearPedido.addEventListener('submit', async (e) => {
      ...
      // Validar aceptación de términos y condiciones
      const checkboxTerminos = document.getElementById('clienteAceptaTerminos');
      const errorTerminos = document.getElementById('errorTerminosCliente');
      ...
      if (!nombreCliente || !telefonoCliente || !direccionEntrega || !descripcion) {
        alert('Por favor completa todos los campos obligatorios');
        return;
      }
