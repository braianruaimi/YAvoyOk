# Script de Deployment Automático SSH para YAvoy
# Ejecuta el deployment completo en Hostinger

$ErrorActionPreference = "Continue"

Write-Host "🚀 Iniciando deployment automático en Hostinger..." -ForegroundColor Cyan
Write-Host ""

# Crear archivo temporal con comandos
$comandos = @"
cd ~/public_html
rm -rf * .[^.]*
git clone https://github.com/braianruaimi/YAvoyOk.git .
npm install
cat > .env << 'EOFENV'
DB_HOST=localhost
DB_PORT=3306
DB_NAME=u695828542_yavoy_web
DB_USER=u695828542_yavoyen5
DB_PASSWORD=Yavoy26!
DB_POOL_MIN=2
DB_POOL_MAX=20
NODE_ENV=production
PORT=5502
JWT_SECRET=YAvoy_Enterprise_JWT_Secret_2024_Ultra_Secure_MySQL
SESSION_SECRET=YAvoy_Session_Secret_2024_MySQL_Enterprise
CSRF_SECRET=YAvoy2026_CSRF_Enterprise_Protection
ENCRYPT_SECRET=YAvoy2026_Encryption_Master_Key
ALLOWED_ORIGINS=https://yavoy.space,https://www.yavoy.space,http://yavoy.space
SMTP_HOST=smtp.hostinger.com
SMTP_PORT=587
SMTP_USER=yavoyen5@yavoy.space
SMTP_PASS=BrainCesar26!
SMTP_SECURE=false
SMTP_TLS=true
VAPID_PUBLIC_KEY=BL4P9zTOxEkQBAmTV3XiyK9305PJDZKoPr52a0NNedpV5OVfuZGlf9SL21zVE9D4AwNfgWzKw8bHA-peL_g-qZs
VAPID_PRIVATE_KEY=SmZvfO1ZhLtbCrewuGFDnG0gfuTeV5DT9vRBjmWlBL4
VAPID_SUBJECT=mailto:yavoyen5@yavoy.space
CEO_USERNAME=admin
CEO_PASSWORD=admin123
EOFENV
npm list -g pm2 || npm install -g pm2
pm2 delete yavoy 2>/dev/null || true
pm2 start server.js --name yavoy
pm2 save
pm2 logs yavoy --lines 50
"@

$tempFile = "$env:TEMP\yavoy-deploy-commands.sh"
$comandos | Out-File -FilePath $tempFile -Encoding ASCII

Write-Host "📝 Comandos preparados en: $tempFile" -ForegroundColor Green
Write-Host ""
Write-Host "🔐 Conectando a Hostinger SSH..." -ForegroundColor Yellow
Write-Host "   Host: 147.79.84.219:65002" -ForegroundColor Gray
Write-Host "   Usuario: u695828542" -ForegroundColor Gray
Write-Host ""

# Ejecutar comandos via SSH
# Nota: Necesitarás ingresar la contraseña: Yavoy25!
ssh -p 65002 -o StrictHostKeyChecking=no u695828542@147.79.84.219 "bash -s" < $tempFile

Write-Host ""
Write-Host "✅ Deployment completado!" -ForegroundColor Green
Write-Host ""
Write-Host "🌐 Tu aplicación debería estar disponible en:" -ForegroundColor Cyan
Write-Host "   https://yavoy.space" -ForegroundColor White
Write-Host "   http://yavoy.space:5502" -ForegroundColor White
Write-Host ""
Write-Host "📊 Para ver los logs, ejecuta:" -ForegroundColor Yellow
Write-Host "   ssh -p 65002 u695828542@147.79.84.219 'pm2 logs yavoy'" -ForegroundColor White
Write-Host ""

# Limpiar archivo temporal
Remove-Item $tempFile -ErrorAction SilentlyContinue
